...
const ProjectsView = () => {
  ...
  const handelDownloadeAttachedFile = async (uniqueFileName) => {
    const downloadUrl = `/download-file/${uniqueFileName}`; // Remove the extra colon

    try {
      // Create an anchor element to trigger the file download
      const link = document.createElement("a");
      link.href = downloadUrl;
      link.download = uniqueFileName; // Optional: set the file name for downloading
      link.click();
    } catch (error) {
  ...
  const triggerRefresh = () => setRefreshKey((prev) => prev + 1); // Increment refreshKey
      ...
      <div className="" ref={componentRef}>
        ...
        <div className="flex flex-col h-fit gap-y-4 gap-x-4 md:flex-row">
          <div className="flex-1 bg-white p-4 rounded-md">
                    ...
                    >
                      ...
                      <Button className="bg-transparent border-primary text-primary">
                        <a
                          href="#"
                          onClick={() =>
                            handelDownloadeAttachedFile(file?.uniqueFileName)
            ...
            <UploadeFile projectId={id} onUploadSuccess={triggerRefresh} />
